...
public partial class OnboardingPage : BasePage, INotifyPropertyChanged
{
    ...
    public string CurrentDescription
    {
        ...
        set
        {
            if (_currentDescription == value) return;
            _currentDescription = value;
            OnPropertyChanged();
        }
    }

    private bool _isLastSlide;
    public bool IsLastSlide
    {
        get => _isLastSlide;
        set
        {
            if (_isLastSlide == value) return;
            _isLastSlide = value;
            OnPropertyChanged();
        }
    }
    ...
    private void UpdateBottomSection(int position)
    {
        ...
        CurrentDescription = _slides[position].Description;
        IsLastSlide = position == _slides.Count - 1;
